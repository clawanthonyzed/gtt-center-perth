...
>
> **INDEPENDENT AUDIT ROUND, 2026-08-18 — SUPERSEDES the figures above, same-day-plus-one, per Anthony's explicit "sanity-audit before proceeding to Phase D" instruction.** The audit found the Venue Manager's wage rate (Clerks Award MA000002 L2, A$36.81/hr) was a likely misclassification — the role's own duties (runs daily venue operations, manages staff, requires a service qualification) match the Hair & Beauty Industry Award MA000005's own Level 6 "salon manager" classification (A$40.00/hr) far more closely, researched and corrected 2026-08-18. A genuine arithmetic error was also found and fixed in the first-principles document's own Saturday treatment-staff hand-calculation (used 4.3333 Saturdays/month for one line instead of this repo's canonical 4.33 — a real bug, not a rounding artifact, immaterial in size but disclosed in full). **CURRENT, authoritative steady-state Net Operating Result: A$48,024.86/month (Table 1) and A$13,934.85/month (Table 2)** — both DOWN again, a genuine cost increase from correcting a real misclassification, not an error introduced by the audit. Workers compensation (1.7%) was re-investigated but the primary WorkCover WA rate source could not be fetched (403 Forbidden, a genuine tooling limitation) — remains PLACEHOLDER, not upgraded to VERIFIED. **The audit did NOT find a fundamental "this business doesn't work" problem** — the 18-client AM timetable is independently solver-verified against the committed 8 treatment staff + 2 phlebotomists with zero collisions (`docs/scenario-c-sync-timetables.md` §0.6a), and the venture remains net-positive at 18 clients/day throughout. Full detail, every line: `docs/architecture/FIRST-PRINCIPLES-FINANCIAL-MODEL.md`.
...
| Total Direct Labor + Opening Costs | **A$89,460.88** (2026-08-18 audit round — Venue Manager wage corrected to MA000005 L6 — was A$88,733.69 under the 2026-08-17 first-principles rebuild, A$82,193.88 under the proportional-scaling recompute — includes current researched wage rates, first-principles Venue Manager/PM Reception costing, superannuation; see `data/canonical/cost_ramp.yml#cost_table1_m5plus`) | `[CALCULATED — data/canonical/cost_ramp.yml, deterministic output of tools/cost_ramp_model.py]` |
| Workers Comp (1.7%) | **A$1,520.83** (was A$1,508.47, A$1,397.30 before that) | `[MODELED — 1.7% convention, applied to the recomputed Direct Labor total; rate itself remains PLACEHOLDER, not VERIFIED — WorkCover WA source blocked automated fetch, 2026-08-18]` |
| Non-Wage Overhead | A$13,980.00 (unchanged — rent/utilities not client-volume-driven) | `[MODELED — unchanged]` |
| **Total Costs** | **A$115,697.02** (was A$114,870.20, A$101,378.78 before that) | `[CALCULATED — 2026-08-18 audit round]` |
| **Net P&L — PRIMARY COMMITTED STEADY-STATE FIGURE** | ~~+A$63,028.75/month~~ ~~+A$56,581.70/month~~ ~~+A$53,837.02/month~~ ~~+A$48,851.68/month~~ **+A$48,024.86/month (2026-08-18 audit round — Venue Manager wage corrected from Clerks Award L2 to Hair & Beauty Award MA000005 L6, a real award-classification finding; all older figures retained struck-through for trace, not deleted)** | `[CALCULATED — data/models/master_financial_model.yml#outputs.steady_state_summary, 2026-08-18]` |
| Quarterly | +A$144,074.58 (was +A$146,555.04) | `[CALCULATED — Monthly × 3]` |
| Half-Yearly | +A$288,149.16 (was +A$293,110.08) | `[CALCULATED — Monthly × 6]` |
| Yearly | +A$576,298.32 (was +A$586,220.16) | `[CALCULATED — Monthly × 12]` |

### SECONDARY — Table 2 (12-client/08:00/g=25), steady state — DOWNSIDE/SENSITIVITY REFERENCE ONLY, not a planning alternative

| Line | Amount | Tag |
|---|---|---|
| Total Revenue (Monthly) | A$124,226.88 (was A$115,720.80 — PM revenue rebuilt from the real A$117 average, see `docs/architecture/PM-PACKAGES.md` §5) | `[VERIFIED — data/models/master_financial_model.yml]` |
| Total Direct Labor + Opening Costs | A$84,707.15 (2026-08-18 audit round, VM wage correction — was A$83,979.96, A$78,570.01 before that) | `[CALCULATED — data/canonical/cost_ramp.yml#cost_table2_m5plus]` |
| Workers Comp (1.7%) | A$1,440.02 (was A$1,427.66, A$1,335.69 before that) | `[MODELED — rate itself remains PLACEHOLDER, not VERIFIED]` |
| Non-Wage Overhead | A$13,980.00 | `[MODELED — unchanged]` |
| **Total Costs** | **A$110,292.03** (was A$109,465.22, A$97,258.43 before that) | `[CALCULATED]` |
| **Net P&L — DOWNSIDE/SENSITIVITY REFERENCE FIGURE** | ~~+A$27,084.69/month~~ ~~+A$21,056.64/month~~ ~~+A$18,462.37/month~~ ~~+A$14,761.66/month~~ **+A$13,934.85/month (2026-08-18 audit round)** | `[CALCULATED, data/models/master_financial_model.yml, 2026-08-18]` |
| Quarterly | +A$41,804.55 (was +A$44,284.98) | `[CALCULATED]` |
| Half-Yearly | +A$83,609.10 (was +A$88,569.96) | `[CALCULATED]` |
| Yearly | +A$167,218.20 (was +A$177,139.92) | `[CALCULATED]` |

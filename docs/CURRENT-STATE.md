...
>
> **PM CAPACITY/TRANSACTION RECONCILIATION, 2026-08-18 (Priority 1) — SUPERSEDES the audit-round figures above, same day.** Building the PM operating timetable (`docs/architecture/OPERATING-MODEL-18-CLIENTS.md`) surfaced a real contradiction: PM revenue was calculated by multiplying 16 "sessions/day" directly by the A$117 average transaction price, but 16 sessions/day is a STAFF-SESSION capacity figure (correct for labour-hours costing), not a CLIENT-TRANSACTION count — PM Refresh and PM Restore packages each consume more than one staff-session per single client transaction. Researched (not assumed) how each package is actually delivered: PM Refresh (Massage+Beauty) uses ONE dual-qualified therapist, consistent with the venue's own existing Massage+Beauty common-pool design; PM Restore (Nails+Hair) genuinely needs TWO specialists, since no dual Nails+Hair qualification is confirmed anywhere in this repo. Full investigation, comparison of viable models, and calculation: `docs/architecture/PM-CAPACITY-RECONCILIATION.md`. Corrected PM transaction capacity: 12.8128/day weekday, 6.4064/day Saturday (was 16/8 staff-sessions used directly) — PM revenue A$36,225.69/month (was A$45,236.88, -19.9%). PM labour cost is UNCHANGED (the same staff-hours capacity drives both figures; only the transaction-counting was wrong). **CURRENT, authoritative steady-state Net Operating Result: A$39,013.67/month (Table 1) and A$4,923.66/month (Table 2)** — Table 2's margin of safety has narrowed to just 0.748 clients/day and its 24-month cumulative result is now negative; Table 1 remains solidly net-positive throughout and is unaffected in its status as the sole planning case. **The 18-client AM timetable itself remains independently solver-verified against the committed 8 treatment staff + 2 phlebotomists with zero collisions** (`docs/scenario-c-sync-timetables.md` §0.6a) — this round's finding was a revenue-calculation error, not an operational feasibility problem. Full detail, every line: `docs/architecture/FIRST-PRINCIPLES-FINANCIAL-MODEL.md`, `docs/architecture/PM-CAPACITY-RECONCILIATION.md`.
...
| Total Revenue (Monthly) — canonical methodology | **A$154,710.69** (2026-08-18, PM capacity/transaction reconciliation — was A$163,721.88 with PM revenue overstated by a session/transaction conflation, see banner above and `docs/architecture/PM-CAPACITY-RECONCILIATION.md`) | `[CALCULATED — data/canonical/revenue_ramp.yml, deterministic output of tools/revenue_ramp_model.py]` |
| Total Direct Labor + Opening Costs | **A$89,460.88** (unchanged by this round's revenue correction — PM labour hours are not affected — 2026-08-18 audit round — Venue Manager wage corrected to MA000005 L6 — was A$88,733.69 under the 2026-08-17 first-principles rebuild, A$82,193.88 under the proportional-scaling recompute; see `data/canonical/cost_ramp.yml#cost_table1_m5plus`) | `[CALCULATED — data/canonical/cost_ramp.yml, deterministic output of tools/cost_ramp_model.py]` |
| Workers Comp (1.7%) | **A$1,520.83** (unchanged this round — was A$1,508.47, A$1,397.30 before that) | `[MODELED — 1.7% convention, applied to the recomputed Direct Labor total; rate itself remains PLACEHOLDER, not VERIFIED]` |
| Non-Wage Overhead | A$13,980.00 (unchanged — rent/utilities not client-volume-driven) | `[MODELED — unchanged]` |
| **Total Costs** | **A$115,697.02** (unchanged this round) | `[CALCULATED]` |
| **Net P&L — PRIMARY COMMITTED STEADY-STATE FIGURE** | ~~+A$63,028.75/month~~ ~~+A$56,581.70/month~~ ~~+A$53,837.02/month~~ ~~+A$48,851.68/month~~ ~~+A$48,024.86/month~~ **+A$39,013.67/month (2026-08-18, PM capacity/transaction reconciliation — a real correction of an overstated PM revenue figure, not a deliberate worsening; all older figures retained struck-through for trace, not deleted)** | `[CALCULATED — data/models/master_financial_model.yml#outputs.steady_state_summary, 2026-08-18]` |
| Quarterly | +A$117,041.01 (was +A$144,074.58) | `[CALCULATED — Monthly × 3]` |
| Half-Yearly | +A$234,082.02 (was +A$288,149.16) | `[CALCULATED — Monthly × 6]` |
| Yearly | +A$468,164.04 (was +A$576,298.32) | `[CALCULATED — Monthly × 12]` |

### SECONDARY — Table 2 (12-client/08:00/g=25), steady state — DOWNSIDE/SENSITIVITY REFERENCE ONLY, not a planning alternative

| Line | Amount | Tag |
|---|---|---|
| Total Revenue (Monthly) | A$115,215.69 (2026-08-18, PM capacity/transaction reconciliation — was A$124,226.88) | `[CALCULATED — data/models/master_financial_model.yml]` |
| Total Direct Labor + Opening Costs | A$84,707.15 (unchanged this round — 2026-08-18 audit round, VM wage correction — was A$83,979.96, A$78,570.01 before that) | `[CALCULATED — data/canonical/cost_ramp.yml#cost_table2_m5plus]` |
...
| **Total Costs** | **A$110,292.03** (unchanged this round) | `[CALCULATED]` |
| **Net P&L — DOWNSIDE/SENSITIVITY REFERENCE FIGURE** | ~~+A$27,084.69/month~~ ~~+A$21,056.64/month~~ ~~+A$18,462.37/month~~ ~~+A$14,761.66/month~~ ~~+A$13,934.85/month~~ **+A$4,923.66/month (2026-08-18, PM capacity/transaction reconciliation — margin of safety has narrowed to just 0.748 clients/day, see §6 of `docs/architecture/FIRST-PRINCIPLES-FINANCIAL-MODEL.md`; Table 2's 24-month cumulative result is now negative — reported plainly, this remains a sensitivity/downside reference only, never the planning case)** | `[CALCULATED, data/models/master_financial_model.yml, 2026-08-18]` |
| Quarterly | +A$14,770.98 (was +A$41,804.55) | `[CALCULATED]` |
| Half-Yearly | +A$29,541.96 (was +A$83,609.10) | `[CALCULATED]` |
| Yearly | +A$59,083.92 (was +A$167,218.20) | `[CALCULATED]` |

...
## 0. FOUNDER DECISION ROUND, 2026-09-19 — Read This First

Per Anthony's direct instruction, four operating-model/financial-model decisions are now settled and propagated through the full canonical layer (`data/canonical/*.yml`, `tools/cost_ramp_model.py`, `tools/revenue_ramp_model.py`, `data/models/master_financial_model.yml`) — not just described differently in prose:

1. **Phlebotomist employment model — CLOSED.** SOLENA (the venture's working operating name) employs both phlebotomists directly (Option A, in-house) and performs the blood collection itself. The pathology partner's role is limited to lab transport and processing only. This closes the "in-house vs partner-supplied" question that was open throughout August 2026 — see §4 below. **No dollar change** — this was already the costed baseline; only the status tag moves from "contingent on an open question" to `[VERIFIED/DECIDED]`.
2. **Venue Manager hours — Monday to Friday (was Monday to Saturday).** Recomputed on a 5-day week. See §4/§5 below. **Genuine open gap, not resolved by this change:** who covers Venue Manager duties (opening, reception oversight, rostering escalation) on Saturday — still a full committed trading day under Table 1 — is not answered here. No new role or cost is invented to cover it.
3. **Demand-driven AM staffing, solver-verified at 5 volumes.** Full rebuild — see §10 below and `docs/architecture/AM-DEMAND-DRIVEN-STAFFING-TIERS-2026-09.md`. Genuine finding: only the lowest tier (9.00 clients/day) can reduce treatment headcount below the committed 8, and only via a widened 45-minute pair cadence (not yet a founder decision, same status as the already-published 6-client finding). 11.29–18.00 clients/day all require the full 8.
4. **PM capacity locked at exactly 10 sessions/day (was 16).** Full session-consumption reconciliation (Model E) — see §5 below and `docs/architecture/PM-SESSION-CAPACITY-MODEL-E-2026-09.md`. PM revenue: was A$36,225.69/month → now **A$24,585.37/month**.

**Net effect on the headline Table 1 (18-client, committed) figures:** Total Revenue A$154,710.69 → **A$143,070.37/month**. Total Operating Costs A$110,544.52 → **A$107,883.97/month**. Net Operating Result (Operating Profit) A$44,166.17 → **A$35,186.40/month**. Break-even AM client volume 11.290/day is now stale — recomputed to **12.655/day** under the committed (always-8-staff) model, or a secondary 8.266/day under a not-yet-adopted demand-flexed low-volume model — see §10.
...
| Role | Qty | Tag |
|---|---|---|
| Venue Manager (Managing Director) | 1 — new hire, not yet in place. **Hours corrected 2026-09-19: Monday–Friday (was Monday–Saturday).** | `[PLACEHOLDER — critical-path hire, recruitment gated on securing a physical venue location, not yet begun]`. Mon-Fri hours: `[DECIDED — Anthony's direct instruction, 2026-09-19]`. **Genuine open gap, not resolved:** Saturday VM coverage (opening/reception oversight/rostering escalation) is unanswered — Table 1 trades 6 days/week — no new role or cost invented to cover it. |
| Phlebotomist (AM only, Chair A / Chair B) | 2 | `[MODELED — financial-break-even-staff.md award-rate structure]`. **CLOSED 2026-09-19, per Anthony's direct instruction:** SOLENA employs both phlebotomists directly (Option A, in-house) — the pathology partner is limited to lab transport/processing only. This closes the "in-house vs partner-supplied" question first flagged 2026-07-30 (Carole Rivers' WDP email raised the possibility WDP might supply/employ the collector instead — that possibility is now settled in favour of in-house employment, not left open). **No dollar change** — the ~A$48,310.68/month AM weekday Direct Labor figure this feeds (§7 below) was already the in-house-costed baseline; only the status tag changes. `[VERIFIED/DECIDED — Anthony's direct instruction, 2026-09-19]` |
...
>
> **FIRST-PRINCIPLES LABOUR REBUILD, 2026-08-17 (Phase C) — SUPERSEDES the wage-rate recompute above, same day.** The wage-rate recompute above scaled the existing labour-cost total proportionally (old rate → new rate scale factor). This second, same-day recompute replaces that bridge methodology with a genuine first-principles build (position → headcount → hours/shift → operating days → wage rate → Saturday penalty → super → workers comp), using `docs/architecture/STAFF-PROFILES.md`'s real position/hours/wage data — see `docs/architecture/FIRST-PRINCIPLES-FINANCIAL-MODEL.md` for the full derivation. The PM average transaction value was also rebuilt from the real service/package mix (A$95 placeholder → A$117 derived figure, `docs/architecture/PM-PACKAGES.md` §5), not just swapped. **Interim current steady-state Net Operating Result at this stage: A$48,851.68/month (Table 1) and A$14,761.66/month (Table 2)** — both DOWN again from the proportional-scaling figures above, because first-principles costing (particularly the Venue Manager's real 8hr AM+admin day and PM Reception's real headcount, both previously modelled as small flat allowances) is materially higher than the proportional-scaled bridge figure.
>
> **INDEPENDENT AUDIT ROUND, 2026-08-18 — SUPERSEDES the figures above.** The audit found the Venue Manager's wage rate (Clerks Award MA000002 L2, A$36.81/hr) was a likely misclassification — corrected to the Hair & Beauty Industry Award MA000005's own Level 6 "salon manager" classification (A$40.00/hr). A genuine arithmetic error was also found and fixed in the first-principles document's own Saturday treatment-staff hand-calculation (immaterial in size, disclosed in full). Interim steady-state Net Operating Result at this stage: A$48,024.86/month (Table 1) and A$13,934.85/month (Table 2). Workers compensation (1.7%) remains PLACEHOLDER, not VERIFIED.
>
> **PM CAPACITY/TRANSACTION RECONCILIATION, 2026-08-18 (Priority 1) — SUPERSEDES the audit-round figures above, same day.** Building the PM operating timetable (`docs/architecture/OPERATING-MODEL-18-CLIENTS.md`) surfaced a real contradiction: PM revenue was calculated by multiplying 16 "sessions/day" directly by the A$117 average transaction price, but 16 sessions/day is a STAFF-SESSION capacity figure (correct for labour-hours costing), not a CLIENT-TRANSACTION count. Researched (not assumed) how each package is actually delivered: PM Refresh uses ONE dual-qualified therapist; PM Restore genuinely needs TWO specialists. Corrected PM transaction capacity: 12.8128/day weekday, 6.4064/day Saturday — PM revenue A$36,225.69/month (was A$45,236.88, -19.9%). Full investigation: `docs/architecture/PM-CAPACITY-RECONCILIATION.md`.
>
> **FINANCIAL FINALISATION, 2026-08-18 (same day) — SUPERSEDED, see the 2026-08-21 banner below.** Two open items resolved by investigation, per Anthony's explicit "investigate, don't ask Anthony to pick" instruction. **(1) Insurance corrected (still current, unaffected by the 2026-08-21 reversal below):** neither A$400/month (an unexplained round guess) nor A$1,279/month (found this round to be methodologically flawed — double-counted workers compensation, already charged separately, and included an optional business-interruption line) was defensible. Corrected to A$708.34/month (Public Liability + Professional Indemnity + Property/Contents only, externally sanity-checked against real 2026 Australian small-business premium research) — non-wage overhead A$13,980.00 → A$14,288.34/month. **(2) Relief/absence coverage added, then REVERTED 2026-08-21 (see banner below) — the payroll model previously priced only the committed roster with zero absences, ever; a new, quantified recurring line (A$6,128.53/month, full-shift relief replacement at an 8% per-person absence-rate planning assumption) was added to represent this real cost, then removed again per direct founder instruction.** Full investigation and derivation: `docs/architecture/FINANCIAL-ASSUMPTION-REGISTER.md`. This round's own steady-state Net Operating Result (A$32,576.80/month Table 1, -A$1,513.21/month Table 2) is no longer current — see the 2026-08-21 banner immediately below for the reverted figures.
>
> **FINANCIAL MODEL REBUILD, 2026-08-21 — SUPERSEDED, see the Founder Decision banner below.** Per direct founder instruction: the relief_absence_allowance line added 2026-08-18 is REVERTED to A$0.00/month — normal payroll/rostering (casual relief hiring, a colleague picking up an extra shift) is treated as already absorbing absence coverage, not a separate expected-value planning line stacked on top of it, unless a genuine, itemised, real cost basis for a separate line is identified (none has been). The insurance correction from the same 2026-08-18 round is UNCHANGED (still A$708.34/month). This round's own steady-state Net Operating Result (A$38,705.33/month Table 1, A$4,615.32/month Table 2) is no longer current — see the banner immediately below for the further-updated figures. Full detail: `data/canonical/cost_ramp.yml#conflict_relief_absence_allowance_reverted`.
>
> **FOUNDER DECISION, 2026-08-21 (later same day) — SUPERSEDES the Financial Model Rebuild figures above, the current, authoritative figures.** Position 06/RCO01 (dedicated PM Reception) is REMOVED as a payroll line, per Anthony's direct instruction — overriding `docs/architecture/STAFFING-COVERAGE-VALIDATION.md` §4's prior Model A recommendation. The confirmed model is Model C (service-staff coverage during PM booking gaps): rostered PM treatment staff handle check-in/payment/Fresha administration during natural gaps in their own paid PM session bookings, already paid for — no separate role or cost. Fresha/payment-system training has been added as a real, explicit requirement on the PM treatment-staff position profiles, not a hand-wave (see `docs/architecture/STAFF-PROFILES.md`). **CURRENT, authoritative steady-state Net Operating Result: A$44,166.17/month (Table 1) and A$10,076.16/month (Table 2 — more than doubles).** Table 2's 24-month cumulative result turns POSITIVE for the first time under any 2026-08-18-or-later cost base (+A$106,006.50 by Month 24), and its cash-flow trough (Month 3, -A$98,125.17) now falls within the historical working-capital reserve's own upper bound. Table 1 remains solidly net-positive throughout (annualised +A$529,994.04). **The 18-client AM timetable itself remains independently solver-verified against the committed 8 treatment staff + 2 phlebotomists with zero collisions** (`docs/scenario-c-sync-timetables.md` §0.6a) — this round's correction is a staffing-structure/cost-line change, not an operational feasibility problem. Full detail: `data/canonical/cost_ramp.yml#change_pm_reception_dedicated_role_removed`, `docs/architecture/STAFFING-COVERAGE-VALIDATION.md` §4, `docs/architecture/DEMAND-DRIVEN-STAFFING-MODEL.md` §5, `data/models/master_financial_model.yml` (2026-08-21 banner).
...
> **FOUNDER DECISION ROUND, 2026-09-19 — SUPERSEDES the Founder Decision (2026-08-21) figures below, current and authoritative.** Per Anthony's direct instruction: (1) Venue Manager Mon-Fri (was Mon-Sat) — opening_time_increment A$9,118.40 → A$7,040.00/month; (2) PM capacity locked at exactly 10 sessions/day (was 16, Model E session-count method supersedes Model C) — PM revenue A$36,225.69 → A$24,585.37/month, PM weekday labour now genuinely floor-bound at every ramp month (flat A$9,808.92/month, no more M5plus anchor); (3) phlebotomist employment model CLOSED (SOLENA employs both directly) — no dollar change, status tag only. Full derivation: `docs/architecture/PM-SESSION-CAPACITY-MODEL-E-2026-09.md`, `docs/architecture/AM-DEMAND-DRIVEN-STAFFING-TIERS-2026-09.md`. **New Net Operating Result: A$35,186.40/month (was A$44,166.17).**
...
| Total Revenue (Monthly) — canonical methodology | ~~A$154,710.69~~ **A$143,070.37** (2026-09-19 — PM capacity corrected to 10 sessions/day, was 16; see banner above and `docs/architecture/PM-SESSION-CAPACITY-MODEL-E-2026-09.md`) | `[CALCULATED — data/canonical/revenue_ramp.yml, deterministic output of tools/revenue_ramp_model.py]` |
| Total Direct Labor + Opening Costs | ~~A$84,658.03~~ **A$82,318.05** (2026-09-19 — Venue Manager Mon-Fri, PM floor-bound at every month; see `data/canonical/cost_ramp.yml#cost_table1_m5plus`) | `[CALCULATED — data/canonical/cost_ramp.yml, deterministic output of tools/cost_ramp_model.py]` |
| Workers Comp (1.7%) | ~~A$1,439.19~~ **A$1,399.41** (2026-09-19 — recomputed on the reduced Direct Labor subtotal) | `[MODELED — 1.7% convention, applied to the recomputed Direct Labor total; rate itself remains PLACEHOLDER, not VERIFIED]` |
| Non-Wage Overhead | **A$14,288.34** (unchanged this round — 2026-08-18 Financial Finalisation round figure, insurance corrected A$400→A$708.34/month, see `docs/architecture/FINANCIAL-ASSUMPTION-REGISTER.md`) | `[MODELLED — BALLPARK-ESTIMATE, not a real quote, broker quotes in motion]` |
| Relief/Absence Coverage Allowance | **A$0.00** (unchanged — reverted 2026-08-21, per direct founder instruction; normal payroll/rostering treated as already absorbing absence coverage; see `data/canonical/cost_ramp.yml#conflict_relief_absence_allowance_reverted`) | `[DECIDED — founder instruction, 2026-08-21]` |
| **Total Costs** | ~~A$110,544.52~~ **A$107,883.97** (2026-09-19 — Venue Manager Mon-Fri + PM 10-session recompute; was A$116,005.36 earlier 2026-08-21; A$122,133.89 under the 2026-08-18 round; A$115,697.02 before that) | `[CALCULATED]` |
| **Net P&L — PRIMARY COMMITTED STEADY-STATE FIGURE** | ~~+A$63,028.75/month~~ ~~+A$56,581.70/month~~ ~~+A$53,837.02/month~~ ~~+A$48,851.68/month~~ ~~+A$48,024.86/month~~ ~~+A$39,013.67/month~~ ~~+A$32,576.80/month~~ ~~+A$38,705.33/month~~ ~~+A$44,166.17/month~~ **+A$35,186.40/month (2026-09-19, Founder Decision round — Venue Manager Mon-Fri, PM capacity locked at 10 sessions/day, phlebotomist employment model closed; all older figures retained struck-through for trace, not deleted)** | `[CALCULATED — data/models/master_financial_model.yml#outputs.steady_state_summary, 2026-09-19]` |
| Quarterly | +A$105,559.20 (was +A$132,498.51) | `[CALCULATED — Monthly × 3]` |
| Half-Yearly | +A$211,118.40 (was +A$264,997.02) | `[CALCULATED — Monthly × 6]` |
| Yearly | +A$422,236.80 (was +A$529,994.04) | `[CALCULATED — Monthly × 12]` |

### SECONDARY — Table 2 (12-client/08:00/g=25), steady state — DOWNSIDE/SENSITIVITY REFERENCE ONLY, not a planning alternative

| Line | Amount | Tag |
|---|---|---|
| Total Revenue (Monthly) | A$115,215.69 (unchanged this round) | `[CALCULATED — data/models/master_financial_model.yml]` |
| Total Direct Labor + Opening Costs | A$79,904.30 (2026-08-21 — Position 06/RCO01 removed, was A$84,707.15) | `[CALCULATED — data/canonical/cost_ramp.yml#cost_table2_m5plus]` |
| Workers Comp (1.7%) | A$1,358.37 (2026-08-21 — recomputed on the reduced subtotal, was A$1,440.02) | `[MODELED — rate itself remains PLACEHOLDER, not VERIFIED]` |
| Non-Wage Overhead | A$14,288.34 (was A$13,980.00) | `[MODELLED — BALLPARK-ESTIMATE]` |
| Relief/Absence Coverage Allowance | A$0.00 (REVERTED 2026-08-21 — was A$6,128.53 under the 2026-08-18 round) | `[DECIDED — founder instruction, 2026-08-21]` |
| **Total Costs** | **A$105,139.53** (2026-08-21, later same day — Position 06/RCO01 removed; was A$110,600.37 earlier the same day; A$116,728.90 under the 2026-08-18 round; A$110,292.03 before that) | `[CALCULATED]` |
| **Net P&L — DOWNSIDE/SENSITIVITY REFERENCE FIGURE** | ~~+A$27,084.69/month~~ ~~+A$21,056.64/month~~ ~~+A$18,462.37/month~~ ~~+A$14,761.66/month~~ ~~+A$13,934.85/month~~ ~~+A$4,923.66/month~~ ~~-A$1,513.21/month~~ ~~+A$4,615.32/month~~ **+A$10,076.16/month (2026-08-21, Founder Decision round — Position 06/RCO01 removed; Table 2's steady-state result more than doubles)** | `[CALCULATED, data/models/master_financial_model.yml, 2026-08-21]` |
| Quarterly | +A$30,228.48 (was +A$13,845.96) | `[CALCULATED]` |
| Half-Yearly | +A$60,456.96 (was +A$27,691.92) | `[CALCULATED]` |
| Yearly | +A$120,913.92 (was +A$55,383.84) | `[CALCULATED]` |
...
**AM Direct Labor, both models — RECOMPUTED 2026-08-17 (Phase C, first-principles rebuild) to A$48,310.68/month (weekday) — was A$50,082.52 under the proportional-scaling recompute, A$48,254.67 before that:** 2 phlebotomists (A$9,075.00) + 8 treatment staff (4 Massage+Beauty A$19,800.00 + 2 Nails A$9,717.84 + 2 Hair A$9,717.84 = A$39,235.68), current researched casual rates (`docs/financial-break-even-staff.md`), 6-hour shift (07:00-13:00), 22 weekdays/month — full derivation `docs/architecture/FIRST-PRINCIPLES-FINANCIAL-MODEL.md` §3a/3b. Headcount is identical (8 dual-qualified + 2 phlebotomists) at both 18-client Table 1 and 12-client Table 2 — see §4. This remains the central finding of the 2026-08-05 rebase: **Table 1 serves 50% more daily clients than Table 2 at literally zero extra weekday labor cost**, because these are fixed-salary FTE roles and the extended day still fits the shift budget — that finding is unaffected by the wage-rate recompute, only the absolute dollar figures moved.
...
## 10. AM Demand-Driven Staffing Profitability Ladder — REBUILT 2026-09-19

> Per Anthony's direct instruction: "SOLENA may employ a full staff pool capable of supporting the maximum operating capacity. However, staff are not all rostered every day. The roster flexes according to client demand... Do not use a flat 8 service staff every day assumption across every client-volume scenario." Full method: `tools/am_volume_tier_staffing_solver.py` (extends the already-published, calibrated concurrency solver to odd client counts and cadence sweeps) + `tools/am_demand_tier_financial_ladder.py` (combines the solved headcount with the canonical wage rates from `tools/cost_ramp_model.py`). Full narrative: `docs/architecture/AM-DEMAND-DRIVEN-STAFFING-TIERS-2026-09.md`. **The 22.50 clients/day sensitivity point used in earlier presentation work is REMOVED entirely, per Anthony's explicit instruction — it exceeds the physical capacity of the current schedule design and must not appear even as an illustrative extrapolation.**

| AM clients/day | Rostered for (whole clients) | Pair cadence | Treatment headcount | Phlebotomists | Monthly payroll total | Monthly total revenue | Monthly total operating cost | **Monthly Operating Profit** |
|---|---|---|---|---|---|---|---|---|
| 9.00 | 9 | 45min (widened — **not yet a founder decision**) | 4 (2 Massage+Beauty + 1 Nail + 1 Hair) | 2 | A$64,705.01 | A$83,827.87 | A$78,993.30 | **+A$4,834.57** |
| 11.29 | 12 | 25min (committed) | 8 (4 Massage+Beauty + 2 Nails + 2 Hair) | 2 | A$93,595.63 | A$98,901.79 | A$107,883.97 | **-A$8,982.18** (loss-making) |
| 13.50 | 14 | 25min (committed) | 8 (4 Massage+Beauty + 2 Nails + 2 Hair) | 2 | A$93,595.63 | A$113,449.12 | A$107,883.97 | **+A$5,565.15** |
| 15.50 | 16 | 25min (committed) | 8 (4 Massage+Beauty + 2 Nails + 2 Hair) | 2 | A$93,595.63 | A$126,614.12 | A$107,883.97 | **+A$18,730.15** |
| **18.00** | **18** | **25min (committed)** | **8 (4 Massage+Beauty + 2 Nails + 2 Hair)** | **2** | **A$93,595.63** | **A$143,070.37** | **A$107,883.97** | **+A$35,186.40 (planning case)** |

**Genuine, disclosed finding:** only the LOWEST tier (9.00/day) can reduce treatment headcount below the committed 8 — down to 4, and only via a widened 45-minute pair cadence, solver-verified feasible within the WDP 10:30am guidance window but not yet a founder decision (same status as the already-published 6-client/day finding). At 11.29 through 18.00 clients/day, 8 treatment staff is genuinely required at every cadence tested. Phlebotomist headcount (2) is unaffected at every tier — structural (2 collection chairs).

**A genuinely counter-intuitive result, disclosed not smoothed over:** 9.00 clients/day is MORE profitable (+A$4,834.57) than 11.29 clients/day (-A$8,982.18), because 11.29 requires the full 8-staff headcount step-up while 9.00 does not — a real "valley" in the ladder between roughly 9 and 12.655 clients/day (see break-even below).

**Rounding convention:** each fractional volume (a continuous average planning figure, same convention this file has always used for revenue) is rounded UP to the nearest whole client for STAFFING purposes only; revenue continues to use the exact fractional volume.

### Break-even — recomputed, the old 11.290/day figure is now STALE

Because headcount now genuinely flexes with volume, there are two distinct, valid break-even points depending on which staffing regime applies — presenting a single number would be misleading:

| Metric | LOW segment (4-staff, 45min cadence, not yet adopted) | **HIGH segment (8-staff, 25min cadence — the standing committed model)** |
|---|---|---|
| Break-even AM clients/day | 8.266 | **12.655** |
| Break-even clients/week | 49.59 | 75.93 |
| Break-even clients/month | ≈217.6 | ≈333.2 |
| % of the 18-client/day committed target | 45.9% | **70.3%** |
| Approx. share of the addressable market (515/week ÷ ≈2,231.7 births/month) | ≈9.75% | **≈14.93%** |

**Quote the HIGH-segment figure (12.655 clients/day, ~70.3% of committed, ~14.93% of addressable market) as "the" break-even** under the currently-committed, always-8-staff model. The LOW-segment figure (8.266/day) is a real, solver-verified secondary finding, applicable only if the widened-cadence, demand-flexed approach is separately adopted for genuinely low-volume days — disclosed, not blended into one misleading average.

**Margin of safety at the 18.00-client planning case:** 5.345 clients/day above the HIGH-segment break-even (≈29.7% of committed volume) — narrower than the pre-rebuild margin (6.710 clients/day, ≈37.3%), a real, disclosed consequence of the lower PM revenue, not offset by the Venue Manager saving.

**Table 2 (12-client secondary reference) post-rebuild:** Net Operating Result is now **+A$1,096.40/month** (was +A$10,076.16) — a thin but still-positive margin, break-even 11.833/day, margin of safety 0.167 clients/day (~1.4%).

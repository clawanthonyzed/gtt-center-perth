...
The Scenario B/C conflict (see [01_conflicts_log.md](01_conflicts_log.md) CONFLICT-03) happened because the capacity/scheduling model changed but the full timetable tables in [operations-manual.md](operations-manual.md), [staff-plan.md](staff-plan.md), and [gtt-clinical-protocol.md](gtt-clinical-protocol.md) weren't all updated at once. [HANDOFF.md](HANDOFF.md) already does a good job of this for the most recent session, but it's a session-handoff document, not a permanent fixture.
...
This session found that `floor-plan-*` files (4 of them) weren't listed in [reading-order.md](reading-order.md) at all — neither as required reading nor reference-only. A document that isn't listed anywhere gives no signal about its currency to a new reader.

**Suggestion:** Audit [reading-order.md](reading-order.md) against [00_document_inventory.md](00_document_inventory.md) (created this session) periodically and add any missing files. The inventory doc can serve as the master checklist for this.

### P4. Version/Date Banner on Every File
Most documents have a `**Version:** X.X | **Date:** YYYY-MM-DD` line near the top — this made the audit significantly easier for files that had it (e.g. [staff-plan.md](staff-plan.md)'s multiple dated supersession banners were easy to follow chronologically). A few files lack this entirely ([workflow.md](workflow.md) has no version/date banner at all, which contributed to it going stale unnoticed).

**Suggestion:** Add the version/date banner convention to any file still missing it, starting with [workflow.md](workflow.md) when it gets its staffing-model rewrite (see [02_issues_and_risks.md](02_issues_and_risks.md) M2).
...
**Suggestion:** A short checklist — "when the client-volume scenario changes, update: [ ] operations-manual.md timetables [ ] gtt-clinical-protocol.md client count [ ] staff-plan.md headcount note [ ] any patient-facing SMS/email copy [ ] financial break-even figures" — attached to [am-capacity-weekend.md](am-capacity-weekend.md) or the new `CURRENT-MODEL.md` (see P1) would reduce the chance of a repeat of CONFLICT-03.

### O4. Reduce Redundant Scenario-C Documentation
There are currently three separate scheduling documents touching Scenario C in some form ([am-capacity-weekend.md](am-capacity-weekend.md)'s staggered version, [scenario-c-sync-timetables.md](scenario-c-sync-timetables.md), and two HTML visual timelines). Once the sync version is confirmed as final, consider consolidating to avoid the kind of drift found in CONFLICT-03 — e.g., archive the staggered tables in [am-capacity-weekend.md](am-capacity-weekend.md) (keep the narrative/rationale sections, drop the superseded table) once [operations-manual.md](operations-manual.md) is rewritten to point to the sync version directly.

---

## Not Recommended (Explicitly Out of Scope This Session)

- **Full rewrite of every "not reviewed this pass" document** — recommended as a follow-up sweep (see [02_issues_and_risks.md](02_issues_and_risks.md) M5), not attempted here to keep this session's scope to what was explicitly requested (Imara rename, known duplicates, and issues found along the way).

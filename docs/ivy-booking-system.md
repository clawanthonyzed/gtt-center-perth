...
> **CLARIFIED 2026-07-30, later same day — not a conflict, a scoping error in how the previous entry framed it.** This policy applies alongside the committed design capacity (12 clients/day — a full optimization search this same day confirmed the true chair-only ceiling is 14, but 14 requires 12 treatment staff not 8 and is financially worse, so 12 remains committed, see `docs/CURRENT-STATE.md` §1/§7) at two different levels, not in conflict with it:
> - **Design/target capacity (12 clients/day):** what the venue is staffed and scheduled to handle at full booking — both chairs synchronized across all 6 daily slots, on a day that actually books out to the ceiling.
> - **Daily rostering (this Chair B policy):** what actually gets staffed on any given real day, based on real confirmed bookings, which may run below the ceiling. If a day's Chair B slots have fewer than 2 confirmed enquiries, Chair B simply doesn't open that day — Chair A still runs as normal.
>
> These aren't competing rules — the enquiry-threshold policy governs day-to-day rostering below the ceiling; 12 clients/day is what the venue is built to handle on days that do fill up. Nothing left for Anthony to decide on this specific point. See `docs/CURRENT-STATE.md` §1 and `docs/VERIFICATION-TRACKER.md` item 1f.

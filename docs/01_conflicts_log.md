...
| CONFLICT-08: multi-way PM-profitability and AM-capacity-figure discrepancies (Phase 6/7) | **Medium-High** (affects headline financial figures in business-plan.md/executive-summary.md) | FLAGGED across `business-plan.md`, `executive-summary.md`, `cash-flow.md`, `profit-loss-tables.md`, `break-even-sensitivity-analysis.md` — not resolved, recommended as a priority reconciliation |
| CONFLICT-09: payment-timing policy inconsistency (full prepayment vs A$30 deposit) | Medium (customer-facing billing/booking policy — must be resolved before any booking flow goes live) | FLAGGED in `docs/pricing-billing-strategy.md` — not resolved |
...
**CONFLICT-08 detail (found during Phase 6/7 pass, 2026-07-19):** Three separate discrepancies surfaced while verifying `business-plan.md`, `executive-summary.md`, `profit-loss-tables.md`, `cash-flow.md`, and `break-even-sensitivity-analysis.md` against Phase 6/7 spec:
1. **PM-shift profitability:** `pm-staffing-roster.md` (2026-07-14) reports the PM shift alone as a loss (-A$4,384/month) at Month 5+ steady state under the pre-Scenario-C (8-client) AM model; `profit-loss-tables.md` v2.0 (2026-07-17, more recent) reports the whole venture profitable at the same steady-state point under the current 10-client Scenario C model. Not reconciled.
2. **Market-sizing figures:** WA births cited as ~18,000 in one document, ~35,000 in `feasibility.md`; GTT screening rate cited as ~78% in one place, "95%+, universal standard of care" in another. Not reconciled.
3. **A fifth AM-capacity-figure variant:** `capacity-pricing-audit.md` (2026-06-11) references a "12-client/2-chair model" as its own headline finding — distinct from both the 8-client Scenario B and the current 10-client Scenario C sync model, and earlier than both. Not reconciled.

None of these three are resolved this session — each is flagged in the specific documents where it was found (`business-plan.md` §4/§6, `executive-summary.md`, `break-even-sensitivity-analysis.md` §1), and collectively logged here as CONFLICT-08 since they share a root cause: this venture's financial/capacity model has been revised multiple times across several sessions, and not every document that cites a capacity or profitability figure has been updated each time the model changed. **Recommended fix:** a single dedicated reconciliation pass (ideally by Bruno/finance function) that traces every capacity and profitability figure in the entire corpus back to one current source of truth, rather than each document citing its own historical snapshot.

**CONFLICT-09 detail (found while building `docs/pricing-billing-strategy.md`, 2026-07-19):** `onboarding.md`'s draft patient-facing confirmation email copy states "full payment is required at time of booking. No deposit option." while `financial-setup.md` (Step 5, Payment flow table) describes an A$30 deposit-at-booking + balance-on-day model via Fresha Pay. These are two different payment policies stated in two different documents, neither marked as superseded. **Not resolved this session** — flagged in `docs/pricing-billing-strategy.md` §Open Items. This needs a single confirmed policy before either document's payment copy is used in a live booking flow, since they currently contradict each other on a customer-facing point (how much is charged, and when).

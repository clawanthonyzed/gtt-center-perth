...
**Item 1, 2026-08-05 — can Table 2 take a 7th pair at 10:25 instead of the uniform-cadence 10:30?** Anthony asked whether breaking the uniform 25-min cadence for one extra pair (10:05 -> 10:25, a 20-min gap) could add a 7th pair/13th-14th client without exceeding the 10:30 guidance boundary. Tested programmatically: **a literal 10:25 insertion collides at the chair level** — client 3 (started 08:25)'s Draw 3 lands exactly 10:25-10:30, clashing with the new pair's Draw 1, confirmed on both chairs (mirrored). Swept every whole-minute candidate 10:10-10:29: only **10:10** and **10:20** are collision-free. Both were headcount-checked via sweep-line peak concurrency AND greedy first-fit (independent methods) — **both give 11 treatment staff (5 Massage+Beauty pool + 3 Nails + 3 Hair), not 8.** **Rejected — does not work at 8 staff at any collision-free insertion point tested.** Table 2 remains 6 pairs/12 clients/8 staff, as stated above. See `docs/CURRENT-STATE.md` §1 for the same finding logged canonically, and `docs/VERIFICATION-TRACKER.md` item 1n.

**REBASE 2026-08-05, per Anthony's direct instruction:** Table 1 and Table 2 above are no longer just the Carole-facing pair — they are now the actual basis for every committed financial figure in this venture (AM revenue, headcount-for-costing, Monthly Net P&L). Table 1 (18-client) is the new PRIMARY committed daily model; Table 2 (12-client) is the SECONDARY reference. The 23-minute-cadence 12/14-client tables elsewhere in this document (§0.6, standard tables) are now HISTORICAL/superseded for financial-basis purposes, retained for trace only. Full P&L: `docs/CURRENT-STATE.md` §5/§7/§8, `docs/profit-loss-tables.md`'s "PRIMARY REBASED MODEL" section.

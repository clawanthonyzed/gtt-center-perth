...
2. **PM model:** now reflects the confirmed model — individual standalone a-la-carte services (~A$95 avg/session, 4-role dedicated casual roster, hours-based costing — `pm-staffing-roster.md`) — not the old "PM Spa Packages" concept (which sold the same A$250/300 package menu standalone). **Set/fixed PM packages are now a committed direction** (`pm-package-structure.md`, 2026-07-20) — not yet incorporated into the ramp figures below, since real package uptake data doesn't exist yet; flagged as a follow-up refinement once package bookings are live.
...
**2026-07-20 (PM packages now committed)** — Updated references from "PM package option is exploratory" to reflect Anthony's confirmation that set/fixed PM packages are now a committed direction (`pm-package-structure.md`, supersedes the deleted `pm-package-exploration.md`). This document's own revenue figures do not yet incorporate package-driven revenue uplift, since no real uptake data exists — flagged as a follow-up refinement once package bookings go live and real data can replace a guess.

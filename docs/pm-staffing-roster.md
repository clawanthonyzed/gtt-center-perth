...
> **SUPERSEDED 2026-07-28 — "4 dedicated PM-only casual hires" model dropped.** Anthony's direct instruction removes the AM/PM staff split entirely — this document's core premise (4 hires dedicated specifically to PM, separate from AM staff) is superseded by a single combined dual-role pool covering both AM and PM shifts. **See `dual-role-staffing-model-2026-07-28.md` for the current model.** This document's Locked Decisions #1 (PM hours 12:00-18:00, still valid as a shift-window definition), #3 (casual employment terms), and #5 (public holidays) remain relevant background; Decision #2 (headcount: "1 dedicated hire per service line") and the entire PM-only cost tables below are superseded — do not use for current headcount/cost planning. Financial figures below (PM-only payroll, PM-only P&L breakdown) are retained for historical trace only.

**Version:** 2.0 | **Date:** 2026-07-14
**Status:** SUPERSEDED 2026-07-28 (see banner above) — retained for historical reference

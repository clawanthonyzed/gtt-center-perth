...
### True Maximum (N_max) Search — RE-CORRECTED 2026-07-31 — dual-qualification re-tested against the real 14-client timing
...
**True chair/phlebotomist-only ceiling: 14 clients/day** (best result found; every other tested search resolution gives 12 or 13). This ceiling uses a bursty two-cluster schedule (two ~46-min arrival bursts separated by a ~2hr09min gap), a materially different shape from the smooth 40-min-cadence rhythm above.

**Corrected method:** the first headcount check (2026-07-30) used a naive per-line multiply — peak concurrency 3 × 4 lines = 12 — and was never tested against Massage+Beauty dual-qualification the way the 10-client and 12-client cases were. Re-tested via direct interval-overlap simulation on the real bursty timing: combining every Massage window and every Beauty window from the 7 chair-A clients into one shared-pool timeline and sweeping for the true peak overlap gives **3, not 6** — the massage-demand cluster (peak 07:45) and beauty-demand cluster (peak 08:50) occur at different clock times, so the pool genuinely shares capacity across them. Nails and Hair have no confirmed dual-qualification pairing, so each stays on its own line at its individually-checked peak of 3.

**True minimum treatment headcount at N=14: 9 (3 Massage+Beauty pool + 3 Nails + 3 Hair), not 12.** `[VERIFICATION NEEDED — Nails+Hair pairing not yet confirmed as hireable]`: if that pairing were confirmed, the same method gives a hypothetical 6 total — not assumed, flagged only.

**Financial verdict, recomputed:** extra revenue from 12→14 (2 clients × A$250 × 22 days) = A$11,000/month. Extra labor cost at 9 staff (A$551,058/yr: 3×A$62,774 Massage+Beauty pool + 3×A$60,456 Nails + 3×A$60,456 Hair) vs the 12-client model's 8 staff (A$492,920/yr) = +A$58,138/yr = **+A$4,844.83/month**, not +A$20,538.33/month as the earlier naive-multiply estimate had it. **Net: +A$11,000 − A$4,844.83 = +A$6,155.17/month if 14 is pursued with the correctly dual-qualification-optimized 9-person roster — better than staying at 12.**

**Conclusion: this reverses the 2026-07-30 "12 remains committed, N_max=14 explored and rejected" finding, which used an uncorrected headcount estimate.** Whether to adopt N=14 as the new committed model is presented here as a finding, not decided unilaterally — this is a real operational change (bursty schedule shape, net 1 additional hire vs the 12-client model specifically on Nails and Hair) that is Anthony's call. See `docs/CURRENT-STATE.md` §1/§4/§7 and `scenario-c-sync-timetables.md` §0.4 for the same finding recorded canonically.

**Downtime-Fill/Early-Release note:** the figures in this document's Downtime-Fill Revenue & Early-Release Saving section above still reflect the smooth 12-client schedule only. Since whether N=14 becomes committed is still Anthony's decision (not yet adopted), no re-derivation against the bursty schedule has been done — flagged as an open follow-up if 14 is adopted, not silently left stale.

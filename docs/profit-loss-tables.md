...
> 5. **Reconciliation, disclosed not hidden:** this does not land on the A$157,792.16 shown in the table below — a **A$2,576.36 gap**. Traced this to the exact same pre-existing weekly-to-monthly revenue-scaling discrepancy already flagged in this document's own Appendix (see "Monthly Total Revenue" row further below, and the 2026-07-30 changelog entry noting a ~A$3,964-4,700 gap "predating this session's work") — the same fixed-size gap appears identically at the 10-client, 12-client, and now 18-client stages, confirming it is a carried-through artifact of the delta-approach's own history, not a new arithmetic error introduced by this rebase. **This document's own established convention (line "§4's delta-approach figure remains the one used as canonical") was, until 2026-08-09, to treat the delta-built figure as canonical, not the first-principles weekly-scaled one.**
>
> **UPDATE 2026-08-09, per Anthony's direct decision — REVENUE ONLY:** this first-principles figure (A$155,215.80, this step) is now the **canonical revenue methodology** going forward, superseding the A$157,792.16 delta-built figure for future revenue modelling — see `docs/architecture/CANONICAL-REVENUE-METHODOLOGY.md`. **This does not change step 9's Net P&L below** — the delta method remains this document's basis for Net P&L/cost figures, which are out of scope for the revenue-methodology decision (no P&L rebuild was authorised). The A$157,792.16 figure and everything built from it in this document (Total Costs, Net P&L) are preserved as historical/inherited, not deleted — see `data/canonical/revenue_assumptions.yml`'s `rev_historical_table1_monthly_inherited`.

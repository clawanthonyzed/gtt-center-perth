...
**Customer departure:** the leaving moment (welcome-card's counterpart) — see §8, this is the single highest-leverage point for the AM→PM flywheel, not just a clinical sign-off.
...
**Why would a GTT customer return?** Not because she was asked to — because the leaving moment (§5, §6) overturned her expectation of what this appointment would be, and the specific, describable thing she experienced created its own reason to come back on a day she chooses.

**What would she remember?** One or two concrete, sensory-specific things, not a general impression — the welcome-card itinerary that meant she never had to ask "what happens next," the specific service she had (a real massage, not "some pampering"), and the leaving gesture (a specific item, not "a goodie bag").

**What would she discover?** That the PM business exists on its own terms, at the leaving moment specifically — not during her GTT visit as an upsell, and not via a later generic marketing message. Per `RETURN-LOOP.md`: upselling during the clinical visit itself undoes trust at exactly the point it's being built.
...
**When does that invitation occur, and who makes it?** At the leaving moment, folded into the same physical object/gesture that ends the visit — not a separate sales conversation, not a follow-up call. The person delivering it is whoever hands over the leaving item (reception or the last treatment staff she saw), not a scripted "upsell" role.

**What does she receive?** The physical leaving object itself (§6, `PREMIUM-POSITIONING.md`'s ~A$9-10/guest costing) plus, separately, one genuinely useful follow-up message once her results reach her doctor — real information, not marketing, with the PM mention riding alongside it once, briefly, per `RETURN-LOOP.md`'s explicit sequencing rule (usefulness first, invitation second).
...
- Hospitality principles — name-based reception, the leaving gesture, no clinical signalling outside the one room that needs it.
...
Working backwards from it, three things in this document do the actual work: first, the schedule itself is engineered so that no client experiences the venue as a batch — she has her own fixed 125-minute shape (§5), and the fact that eight other women are moving through the same building at staggered intervals never surfaces to her directly. Second, the two moments that cost the least and matter the most — the welcome/itinerary card and the leaving gesture (§6, §8) — are deliberately not volume-sensitive; they cost the same, personal amount whether she is client 1 or client 18 that day. Third, staffing capacity for 18/day is already identical to 12/day (§11) — meaning the "high-volume" side of the question is solved on paper without asking any single staff member to serve more people per hour than the schedule already assumes; nobody has to rush to make 18 work. The honest remaining risk is not the schedule or the staffing model — both are already verified — it's whether the *feeling* of individual attention actually survives contact with a fully-booked real morning, which is precisely why §16's first two recommendations (the reception handover gap, the nail-duration conflict) matter now, before volume, rather than being discovered at it.

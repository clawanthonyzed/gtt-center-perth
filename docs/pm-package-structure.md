# GTT Center Perth — PM Package Structure (Confirmed Direction)

**Status:** **CONFIRMED DIRECTION (reaffirmed 2026-07-30)** — the PM Duo/Refresh/Glow package menu concept is confirmed, not merely proposed; this is no longer a "committed direction, then parked" ambiguity. Anthony confirmed: PM will offer both individual standalone a-la-carte services (existing, unchanged) AND set/fixed PM packages (new), with an active strategy to steer/upsell clients toward the package option since it drives higher revenue per PM visit. **Dollar pricing below is still proposed and modelled, NOT locked — final pricing sign-off from Anthony remains the one open item before this goes live in Fresha or any client-facing material.** Consistency-checked 2026-07-21 against Anthony's requirement that PM packages mirror AM's fixed-price/fixed-contents model — confirmed already consistent, no changes needed. See `docs/CURRENT-STATE.md` §2 for the current status summary.
...
**2026-07-21 (consistency check, then parked)** — Anthony confirmed PM packages should mirror AM's model in that both price AND contents are set/fixed (no client choice in either dimension). Checked against this document: **consistent already** — all 3 proposed packages (PM Duo, PM Refresh, PM Glow) are fixed 2-service pairings at a set price, with no client-choice mechanism in either dimension, matching AM's Package 1 (also fully fixed) rather than AM's Package 2 (client-choice composition). No changes required. **Per Anthony's direction, PM packages are not a current priority — parking this document as-is. No further combos, pricing iterations, or build-out until Anthony reprioritizes.**

**2026-07-30 (reaffirmed as confirmed direction, per Anthony's direct instruction)** — Moved status from "committed direction, then parked" ambiguity to unambiguous **confirmed direction**. Pricing itself remains the one explicit open item, unchanged from before — final package prices still require Anthony's sign-off before going live. Updated `docs/CURRENT-STATE.md` §2 to reflect this status.

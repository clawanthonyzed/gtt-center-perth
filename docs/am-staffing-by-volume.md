...
> **NOTE 2026-07-28 (corrected same day, not superseded):** this document's AM-only verified timetables and Massage+Beauty pooling logic remain fully valid and unaffected — the AM shift stays a distinct, separately-rostered shift, not merged with PM (an earlier same-day version of this note incorrectly implied AM/PM were merging into one combined pool — corrected, see `dual-role-staffing-model-2026-07-28.md` v2.0). Anthony's actual instruction extends this document's Massage+Beauty pooling *principle* — hiring people qualified across multiple service specialties — to also seek Hair+Nail dual-qualified candidates where genuinely available, within the AM shift as structured here. The AM-side headcount minimums calculated here (7-8 treatment staff depending on volume) stand unchanged.

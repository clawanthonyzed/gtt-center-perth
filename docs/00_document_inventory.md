...
> **PRIMARY HUMAN-READABLE DOCUMENT, 2026-08-16 — [`outputs/master-dossier/index.html`](../outputs/master-dossier/index.html).** A standalone, self-contained, 18-chapter HTML document covering the entire venture — business idea, brand, market, services, operating model, venue, startup costs, staffing, wages, financial model, sensitivity, legal/compliance, operations, marketing, expansion, execution status, master action plan. Real tables/charts, full per-chapter source traceability, print-ready for A4 (PDF also committed: `outputs/master-dossier/GTT-Center-Perth-Master-Dossier.pdf`). This is now the #1 entry in the reading path — read it before anything else below.

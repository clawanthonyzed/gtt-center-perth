...
| 1e | 6th client/chair (12 clients/day) — solver-checked against WDP's 10:30am start-time guidance | Anthony (decide whether to pursue), referral pipeline (demand), WDP (rental/volume terms) | **CHECKED, not committed** | Solver-verified 2026-07-30: clears cleanly at the original 8-staff baseline (last Draw 1 10:20am, zero concurrency/collision violations), but the 7-staff/6-staff pooling reductions from 2026-07-29 do NOT hold at 12/day. This answers "does it schedule," not "should we do it" — departure time extends to ~12:48pm (vs ~12:08-12:25pm at 10/day), a real operational cost not yet weighed. See `docs/CURRENT-STATE.md` §1. |
| 1f | Chair B opening policy — enquiry threshold | Settled (Anthony's decision) — booking-system build is the remaining open item | **SETTLED, booking-flow mechanics partially open** | Chair A default/guaranteed; Chair B opens only on a 2nd enquiry for the same slot. See `ivy-booking-system.md` for the full flow. **Not yet specified:** what happens to a lone first enquiry if no 2nd arrives (wait duration, cutoff, alternative-slot offer) — flagged there as a follow-up booking-system design decision, not invented. |

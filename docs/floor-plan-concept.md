...
1. **Patient flow is one-way:** Entry → Reception → Lounge → Collection Room → Treatment Room → Exit. No backtracking.
...
| Blood Collection Room 1 | 18 | **Solid walls** | 1 phlebotomy chair (Chair A), centrifuge, medical fridge, sharps, clinical sink. **Founder decision, closed 2026-08-27: split into 2 separate rooms, one per phlebotomist, for extra client privacy**, overrides the evidence-based "1 room, 3-chair-ready" recommendation (`docs/architecture/FIT-OUT-PROGRAM-DECISION-ANALYSIS.md` Part B). See `docs/architecture/MASTER-PROCUREMENT-SHOPPING-LIST.md` Section E for the full propagated consequence, including genuinely open questions (shared vs duplicated centrifuge/specimen fridge, growth path) not resolved here. |
| Blood Collection Room 2 | 18-20 | **Solid walls** | 1 phlebotomy chair (Chair B), same clinical fit-out as Room 1. sqm estimate per `docs/architecture/FIT-OUT-PROGRAM-DECISION-ANALYSIS.md` Part B's own Option 2 minimum-floor-area evidence, not newly invented. Exact figure requires re-derivation once a venue exists. |
...
| Spray Tan Booth | — | — | **REMOVED FROM THE CONCEPT ENTIRELY (2026-08-16, per Anthony's explicit instruction — too many things in one location) — supersedes the 2026-07-31 "Phase 2" reclassification. See `equipment-costs.md` §2. Not part of any future phase.** |
...
| **DAY-ONE SUBTOTAL** | **~257-259sqm** | | Up from ~239sqm, +18-20sqm for the 2nd Blood Collection Room per the founder decision above (2026-08-27). The ~239sqm figure's own prior derivation (+8sqm Nails, +30sqm Hairdressing, -6sqm Spray Tan removed) is unchanged; this is a further addition on top of it, not a recompute of it. **Floor area only, mechanically added from the already-evidenced ~18-20sqm second-room estimate; the day-one fit-out DOLLAR cost below is NOT recalculated for this larger footprint, flagged as an open item requiring re-derivation once a venue exists, not invented here.** |
...
| **NEW TOTAL (day-one + growth reservations)** | **~280-282sqm** | | Up from ~262sqm, same +18-20sqm 2nd Blood Collection Room addition as the day-one subtotal above (2026-08-27 founder decision). The ~262sqm figure's own prior derivation (hair/nail day-one expansion outweighing growth-reservation reduction and spray-tan removal) is unchanged; this is a further addition on top of it. |
...
╠══════════════╦═══════════════════════════════════════════════════════╣
║              ║                                                       ║
║  RECEPTION   ║            GTT LOUNGE  (32sqm)                       ║
║  (12sqm)     ║   8 reclining chairs, TV, ambient lighting           ║
║              ║   cafe counter along back wall                        ║
║  retail wall ╠═══════════════════╦══════════════════════════════════╣
║              ║  BLOOD COLLECTION  ║   HAIRDRESSING AREA  (20sqm)   ║
╠══════════════╣  ROOM  (22sqm)     ║   2 styling chairs              ║
║              ║                    ║   1 backwash basin               ║
║  CAFE /      ║  Chair A  Chair B  ║   mirror wall, back bar         ║
║  REFRESH.    ║  centrifuge        ╠══════════════════════════════════╣
║  COUNTER     ║  medical fridge    ║   NAIL STATION AREA  (20sqm)   ║
║  (8sqm)      ║  clinical sink     ║   3 stations + LEV extraction   ║
║              ║  privacy screens   ║   separate ventilation zone      ║
╠══════════════╬═══════════════════╬══════════════════════════════════╣
║  TREATMENT   ║  TREATMENT ROOM 2  ║  TREATMENT 3  ║  TREATMENT 4   ║
║  ROOM 1      ║  MASSAGE / FACIAL  ║  FACIAL /     ║  FACIAL /      ║
║  MASSAGE     ║  (14sqm)           ║  BEAUTY       ║  BROWS / LASH  ║
║  (14sqm)     ║                    ║  (12sqm)      ║  (12sqm)       ║
║  table, sink ║  table/bed, sink   ║  bed, sink    ║  bed, mirror   ║
╠══════════════╬════════════════════╬═══════╦═══════╩════════════════╣
║  SPRAY TAN   ║  STAFF ROOM        ║ WC 1  ║ WC 2  ║  LINEN STORE   ║
║  BOOTH (6sqm)║  (10sqm)           ║ ACCESS║  STD  ║  CLEAN (6sqm)  ║
║  extractor   ║  lockers, table    ║(7sqm) ║(4sqm) ╠════════════════╣
║              ║  kettle, sink      ║       ║       ║  DIRTY / BIO   ║
║              ║  STAFF WC (4sqm)   ║       ║       ║  (4sqm)        ║
╚══════════════╩════════════════════╩═══════╩═══════╩════════════════╝
...
## Critical Design Requirements — Architect Brief

### Blood Collection Room

> **Cross-checked 2026-08-06 against Carole Rivers' (WDP) detailed ACC/NPAAC room spec — 5 gaps found, none previously flagged in this document.** Full compliance table and sourcing: `docs/VERIFICATION-TRACKER.md` item 1p. Checked explicitly against every item on Carole's list, not assumed compliant. The 5 new bullets below (marked **GAP, added 2026-08-06**) are concept-level flags for the architect brief, not a redesign — existing bullets are unchanged.

- **Clinical sink** — elbow-operated tap (infection control), not a bathroom vanity.
- **Privacy screens or fixed curtain track** between Chair A and Chair B.
- **Medical-grade refrigeration** — specimen fridge with temperature logging (not a bar fridge).
- **Centrifuge bench** — vibration-resistant solid bench, minimum 600mm deep, lockable under-bench storage.
- **Sharps disposal** — 2x sharps container wall brackets (bench level + stand level), yellow biohazard bins recessed if possible.
- **Lighting** — clinical LED downlights over each phlebotomy chair, colour rendering Ra >90.
...
- **GAP, added 2026-08-06 — reclining chair or examination couch for vasovagal episodes.** Not specified anywhere in this document. Carole's spec explicitly calls for a reclining chair or exam couch separate from the 2 phlebotomy chairs, for a client who feels faint or has a vasovagal reaction during/after venepuncture. **Confirmed missing, exactly as flagged before the check** — needs adding to the Blood Collection Room brief, likely a small additional footprint (~2-3sqm) not yet reflected in the 18sqm target above.
- **GAP, added 2026-08-06 — wheelchair accessibility within the Collection Room itself.** Building-level accessibility is addressed elsewhere (Accessible WC per AS 1428.1, Reception's low counter section, 1.5m-minimum Circulation hallway) — but nothing in this document confirms the Collection Room's own doorway width, internal turning circle, or chair positioning accommodates a wheelchair/mobility-aid user. Needs explicit confirmation at architect brief, not assumed covered by the building-level provisions.
- **GAP, added 2026-08-06 — room ventilation not specified.** Lighting is specified (LED, Ra>90); general room ventilation/air handling for the Collection Room itself is not — ventilation is only discussed elsewhere in this document for the Nail Station's LEV extraction, a different requirement. Needs an explicit ventilation spec for this room.
- **GAP, added 2026-08-06 — impervious bench/floor surface not explicitly stated for this room.** The Treatment Rooms section specifies "smooth, impervious, non-slip" flooring, but scopes it to Facial/Beauty Rooms and the Nail Station Area only — the Blood Collection Room's own bench and floor surface imperviousness is not explicitly called out here, despite being the room with the strongest clinical rationale for it (venepuncture, specimen handling).
- **GAP, added 2026-08-06 — data/power points not specified.** Centrifuge, medical fridge, and any EMR/booking-system terminal in this room all need power and likely data points — not itemised anywhere in this document (contrast with the Nail Station Area, which explicitly specifies "minimum 2x 10A GPOs" per station).
- **PARTIAL, added 2026-08-06 — general storage cupboards.** Only "lockable under-bench storage" tied to the centrifuge bench is specified. NPAAC's own premises standard (C1.1(iv)) calls for a "suitable, secure storage area for supplies... accessible only to staff" — broader than just centrifuge-adjacent storage. Not confirmed whether the existing under-bench provision is sufficient or whether a separate supply storage cupboard is needed in this room.
...
### GTT Lounge
- **Natural light** — client priority. Pregnant women sitting 2+ hours need non-clinical atmosphere.
- **Reclining chairs** — pregnancy recliners with adjustable back (wedge pillow compatible, supports left-lateral tilt).
- **TV + audio** — screen for streaming content, good quality background audio system.
- **Cafe counter** — glucose drink dispensed here (T+5); herbal tea, water, snacks available from T=0.
- **Phone charging** — USB-A and USB-C charging point at every chair (2-hour wait).
- **Privacy screens** — adjustable between chairs for clients who prefer privacy during services.
- **Warm lighting** — dimmable. No harsh clinical downlights (migraine / nausea sensitivity in pregnant clients).
...
- **Counter** — standard reception height (900mm) with low-section for wheelchair/pram (750mm) approach.
- **Retail display** — wall shelving visible to clients in lounge (Gaia, Weleda, Mustela product range).
- **Card payment + POS** — counter space for EFTPOS terminal, receipt printer, dual-screen workstation.

### Accessible WC
- Compliant with AS 1428.1 (Design for access and mobility).
- Mandatory for pregnant patient cohort — many will have mobility aids or be in late pregnancy.
- Confirm compliance with local council building permit prior to fit-out.
...
## Next Steps for Floor Plan Development
...
| WorkSafe WA — nail LEV system pre-application | Architect | Before fit-out commences |
| Obtain 3 fit-out quotes from Perth commercial builders | — | After concept drawings approved |

**Suggested Perth commercial fit-out / interior design contacts:**
- Hames Sharley Perth (hames.com.au) — healthcare + commercial specialist
- Buchan Group Perth (buchan.com.au) — retail + medical fit-out
- Spacemaster Interiors Perth — commercial medical/wellness specialist
- Request referrals via Commercial Real Estate agent appointed at venue
...
**2026-08-27 (Blood Collection Room split into 2, founder decision, closed):** Anthony decided 2 Blood Collection Rooms, one per phlebotomist, for extra client privacy, overriding the evidence-based "1 room, 3-chair-ready" recommendation in `docs/architecture/FIT-OUT-PROGRAM-DECISION-ANALYSIS.md` Part B (Round 3). Split the single 18sqm Blood Collection Room row into 2 rooms (Room 1: 18sqm, Chair A; Room 2: 18-20sqm, Chair B, using the already-evidenced second-room floor-area estimate from that same document, not a newly invented figure). Day-one subtotal ~239sqm -> ~257-259sqm; combined day-one + growth-reservation total ~262sqm -> ~280-282sqm. This is a floor-area addition only, mechanically derived from an existing evidenced estimate, not a cost recalculation; the day-one fit-out dollar cost below (A$228,142-457,559) has not been recomputed for the larger footprint, flagged as an open item pending a confirmed venue, not invented. Full propagated consequence (equipment quantities, genuinely open questions on shared vs duplicated centrifuge/specimen fridge/vasovagal recliner, growth path) recorded in `docs/architecture/MASTER-PROCUREMENT-SHOPPING-LIST.md` Section E.

**2026-08-06 (Carole's detailed ACC/NPAAC room spec cross-checked against this document, per Anthony's instruction — 5 gaps found)** — Carole Rivers' (WDP) 2026-08-06 reply included a full room-requirement list for the collection room. Checked explicitly against this document's Blood Collection Room spec, item by item, not assumed compliant: private room (confirmed), >=10sqm (confirmed, 18sqm), 1-2 bleeding chairs (confirmed), handwashing basin (confirmed as clinical sink), sharps/clinical waste (confirmed), proximity to waiting room (confirmed), privacy/acoustics (confirmed). **5 gaps found, none previously flagged:** reclining chair/exam couch for vasovagal episodes (missing entirely, exactly as suspected before the check), room-specific wheelchair accessibility (building-level only, not room-level), room ventilation (not specified, only Nail Station LEV is addressed elsewhere), impervious bench/floor surface (specified for other rooms, not this one), data/power points (not itemised for this room). One partial: general storage cupboards beyond the centrifuge under-bench provision, per NPAAC C1.1(iv)'s broader "secure storage area for supplies" wording. Added all 5 as concept-level flags under Blood Collection Room in Critical Design Requirements — not a redesign, per Anthony's instruction. Full sourcing and the seated/no-walking-requirement research (separate question, same Carole reply) logged in `docs/VERIFICATION-TRACKER.md` item 1p (room spec) and item 1q (seated-requirement research).

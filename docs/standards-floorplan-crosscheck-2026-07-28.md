...
**Threshold question — RESOLVED 2026-07-29 by Anthony's decision.** §2 (Application) states the Code covers "all persons who perform skin penetration procedures" and "the premises in which such procedures are performed," with an explicit exemption list: dentists, medical practitioners, podiatrists, and registered nurses — phlebotomists are not on this list, so the plain text left this genuinely ambiguous for the Blood Collection Room. **Anthony's decision closes this out: the Blood Collection Room falls under its own standards (NPAAC/pathology-specific governance), not the WA Skin Penetration Code.** Not re-litigated as open — this is the standing decision. Practical effect: the Blood Collection Room's sink/premises requirements should be sourced from NPAAC (and WDP's own Licensed Collection Centre spec, once obtained) rather than from §7 of this Code — see §3 below, which is why NPAAC access now matters more than before.
...
## 3. NPAAC Guidelines for Approved Pathology Collection Centres (3rd Ed.) — **STILL COULD NOT BE RETRIEVED (8 total attempts, updated 2026-07-29)**

**Genuinely not obtained** despite 8 real attempts across two sessions — the original 6 (2x direct PDF fetch at different sub-paths, 1x HTML resource page, 1x digitalhealth.gov.au implementer hub mirror, 1x older health.gov.au mirror, 1x targeted WebSearch for quoted text), plus 2 more on 2026-07-29 via different domains entirely (nanopdf.com mirror — page returned "under maintenance"; nrlquality.org.au knowledge-base page — only linked back to the same unreachable safetyandquality.gov.au PDF, no independent copy). Every safetyandquality.gov.au-domain attempt across both sessions timed out (60s) or connection-reset. **This is not being silently skipped or filled in from general knowledge** — no line-by-line comparison is possible without the actual text.

**What was surfaced via search-summary fragments (2026-07-29, not independently read from the primary source — flagged as secondary, weaker evidence than a direct read):** "Collection premises must comply with all applicable laws and regulations. The current approval certificate must be visibly displayed... hours of operation should be displayed"; "Where complicated procedures such as therapeutic venesection, the injection or infusion of any substance are performed, resuscitation equipment must be provided"; "Specimens must not be stored with food, drink or pharmaceuticals." These are genuine search-engine extractions attributed to the real document, not invented, but not confirmed by this agent directly reading the primary source — treat with appropriate caution.

**Now more consequential given Anthony's 2026-07-29 decision** (Blood Collection Room governed by NPAAC/pathology-specific standards, not the WA Skin Penetration Code — see §1 above): NPAAC is now the sole standard this repo needs for that room's compliance, and it remains unread in full. **Recommendation unchanged and now higher-priority: prioritise getting WDP's actual room spec directly (via Carole Rivers, now engaged) over further NPAAC PDF fetch attempts** — WDP's own Licensed Collection Centre requirements (built on top of NPAAC) are the more direct, more likely-to-succeed path to a real, GTT-specific answer, and sidestep the access problem entirely.

**Section count: 0 meets, 0 doesn't-meet, cannot assess (document inaccessible after 8 attempts) — flagged honestly, not guessed.**
...
**2026-07-29 (v4) — Blood Collection Room governance decided; 2 more NPAAC attempts made.** Anthony's decision closes out the §1 threshold question: Blood Collection Room falls under NPAAC/pathology-specific standards, not the WA Skin Penetration Code — no longer flagged as unresolved. Made 2 further NPAAC access attempts via different domains (nanopdf.com mirror — under maintenance; nrlquality.org.au — no independent copy) — still inaccessible after 8 total attempts. Surfaced a few additional search-derived fragments (not independently read from primary source, flagged as weaker evidence). Recommendation reinforced: WDP's own room spec (via Carole Rivers) is now the more important and more achievable path than further NPAAC fetch attempts, since NPAAC is now the sole applicable standard for that room per Anthony's decision.

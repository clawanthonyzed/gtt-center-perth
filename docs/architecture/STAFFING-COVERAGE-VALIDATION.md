...
> **SUPERSEDED BY FOUNDER DECISION, 2026-08-21 — Model C is now the confirmed model, overriding this section's original 2026-08-18 recommendation of Model A.** Anthony's direct instruction: service-staff coverage during PM booking gaps is the confirmed model. This section's original analysis and conclusion (below, unchanged) are retained in full for trace, per this repo's "never delete, mark superseded" convention — the reasoning was real and well-evidenced at the time, not a mistake, and the specific risks it raised (Fresha/payment training gap, PM-capacity self-defeat, customer-experience consistency) are the reasons this round's implementation closes them explicitly rather than ignoring them: Fresha/payment-system training is now an explicit, real requirement on the PM treatment-staff position profiles (`docs/architecture/STAFF-PROFILES.md`, `docs/architecture/DEMAND-DRIVEN-STAFFING-MODEL.md` §1), not a hand-wave. The PM-capacity self-defeat risk (pulling a treatment staff member off a paid session to do reception) is accepted as a known, disclosed trade-off of the founder decision, not resolved away. Position 06/RCO01 (dedicated PM Reception) is removed from the payroll model as of this decision — see `data/canonical/cost_ramp.yml#change_pm_reception_dedicated_role_removed` and `data/models/master_financial_model.yml` for the full financial propagation. §4a below (the relief-pool recommendation for Model A) is now moot, since there is no dedicated role left to provide relief for — retained for trace, not deleted.

**Four models compared on operational practicality and customer experience, not cost alone (original 2026-08-18 analysis, retained for trace):**

| Model | Description | Assessment (as originally reasoned, 2026-08-18) |
|---|---|---|
| **A — Dedicated PM Reception (was the current model, 2026-08-18)** | 1 person, 13:00-18:00, sole responsibility is reception/booking/payment | Was Recommended at the time. Matches real Perth day-spa comparable practice (Le Beau, Keturah, endota — single reception desk at this scale, per `FIRST-PRINCIPLES-FINANCIAL-MODEL.md` §2a). Customer experience: consistent single point of contact for the whole PM window, no service interruption to treatment staff. **SUPERSEDED 2026-08-21 by direct founder decision — no longer the model.** |
| **B — VM extends to cover 15:00-18:00 instead** | VM works a longer shift, no separate PM Reception role | **Rejected, unchanged.** Would require the VM to work an 11-hour day (07:00-18:00) every trading day — a real fatigue/retention risk for the venue's single most critical role, and removes the VM's own current 2-hour AM-wind-down/admin block (rostering, P&L review, pathology liaison) which has no other home in the schedule. |
| **C — Treatment staff assist with reception between clients** | No dedicated PM reception; a rostered treatment staff member handles check-in/payment during gaps | Was Rejected at the time: treatment staff not trained in Fresha/payment-system administration as a primary skill, and pulling a treatment-staff member off the floor for reception duties during their own paid PM session window directly reduces the PM treatment capacity the whole PM revenue model depends on — self-defeating for a role whose entire purpose is generating PM revenue; inconsistent customer experience. **CONFIRMED 2026-08-21 by direct founder decision, overriding this rejection.** The training gap is now closed (Fresha/payment training added as a real staff-profile requirement); the PM-capacity trade-off and customer-experience-consistency risk are accepted, not resolved away. |
| **D — Relief/part-time PM reception (no fixed daily person)** | A rotating pool covers PM reception, no single named person most days | Was Rejected as the PRIMARY model, adopted as the relief mechanism for Model A. **Moot as of 2026-08-21** — Model A no longer exists, so there is nothing left for this to provide relief for. Retained for trace only. |

**Original conclusion (2026-08-18, now superseded): "Model A (dedicated PM Reception) remains the recommended structure — confirmed, not just re-stated, against 3 real alternatives with their trade-offs shown."**

**Current conclusion (2026-08-21): Model C is the confirmed model, per Anthony's direct founder decision. This overrides the original recommendation above. The original reasoning is not deleted because it was wrong — it was a genuine, evidenced analysis of a real trade-off; the founder decision accepts that trade-off explicitly (PM treatment capacity is reduced during reception-covering gaps, and customer experience during those specific moments is less consistent than a dedicated desk) in exchange for not carrying a dedicated payroll line for the role. See `docs/architecture/DEMAND-DRIVEN-STAFFING-MODEL.md` §5 for the full disclosure of this decision and what was required before implementing it.**

### 4a. PM Reception Relief — the Genuinely Open Item, Now Given a Recommendation

> **MOOT as of 2026-08-21 — Model A (the role this relief pool was designed to back up) no longer exists.** Retained for trace only, not deleted.
...
| PM Reception (Position 06/RCO01) | 1 | 1, no relief sized | **REMOVED 2026-08-21 — no dedicated role, per founder decision; service-staff coverage during PM booking gaps confirmed (Model C)** | §4 (superseded) |

**Financial impact of this recalculation itself: NONE — this is a headcount-establishment recommendation (who to have EMPLOYED and on the books), not a change to the STEADY-STATE PAYROLL figure.** Per the five-concepts distinction (`FIRST-PRINCIPLES-FINANCIAL-MODEL.md` §9), relief-pool staff are only paid when they actually work a shift covering an absence — increasing the recommended employed headcount from "10-11"/"3-4" to "12"/"4" does not itself change `data/canonical/cost_ramp.yml`'s payroll figures, which already correctly model only the COMMITTED simultaneous headcount (8 treatment + 2 phlebotomists) working their full rostered shift. **Separately, PM Reception's removal (2026-08-21, §4 above) is a genuine, real change to steady-state payroll** — a direct founder decision, not a recalculation of this section's own binomial-reliability method; see `data/canonical/cost_ramp.yml#change_pm_reception_dedicated_role_removed` for that specific financial propagation.
...
| PM Reception (Position 06/RCO01) — **REMOVED 2026-08-21** | Clerks Award MA000002, Level 1 | A$33.71/hr casual | 5hr PM shift | x1.5 penalty | 12% | 1.7% | 25% casual loading (embedded) | **Role no longer exists — REMOVED per founder decision, §4 above** | **RESEARCHED-BEST-EVIDENCED (2026-08-16 research), now historical** — this wage research is retained for trace only; the role itself is removed from the committed payroll model as of 2026-08-21. Fresha/payment training is now a requirement on the PM treatment-staff roles instead, not this position. |
...
**2026-08-21 (PM Reception Model C confirmed by founder decision, overriding this document's own Model A recommendation)** — Anthony's direct instruction: service-staff coverage during PM booking gaps (Model C, §4) is the confirmed model. §4 updated to mark the original Model A recommendation SUPERSEDED BY FOUNDER DECISION, with today's date and this context -- the original 4-model comparison and reasoning retained in full for trace, not deleted. §4a's relief-pool recommendation marked MOOT (nothing left to provide relief for). §5's headcount table and §6's wage table updated to show Position 06/RCO01 REMOVED. Fresha/payment-system training added as a real requirement on the PM treatment-staff position profiles (`STAFF-PROFILES.md`), closing the training-gap risk this document's original Model C rejection identified -- not resolved away, explicitly addressed. Financial propagation (payroll, P&L, cash flow, break-even) is a real change this round, tracked in `data/canonical/cost_ramp.yml#change_pm_reception_dedicated_role_removed` and `data/models/master_financial_model.yml`, not invented here.

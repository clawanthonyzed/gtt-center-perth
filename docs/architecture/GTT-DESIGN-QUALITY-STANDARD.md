...
3. **"Awesome Design"**: resolved to a category of curated reference-list repositories (e.g. `awesome-claude-skills`), not a single installable skill. Not installed as its own skill, since installing a meta-list would be redundant with the 2 skills above; noted as a future reference source if a specific new design-tooling need arises.
4. **Image-to-Code**: identified a real candidate (`anydesign`, analyses an image/URL/Figma file into a structured design system and component inventory) but not installed this session, given the time already spent on higher-priority parts of this task. Flagged as the next step if a screenshot-to-code workflow is genuinely needed.
